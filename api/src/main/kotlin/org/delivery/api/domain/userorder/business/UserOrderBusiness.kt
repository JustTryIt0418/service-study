...
class UserOrderBusiness(
    ...
    fun userOrder(
        ...
        //메뉴 찾기
        val storeMenuEntityList = request?.storeMenuIdList
        ...
        return userOrderService.current(user?.id)
            .map { userOrderEntity ->
                val storeMenuEntityList = userOrderEntity.userOrderMenuList?.stream()
                    ?.filter { userOrderMenuEnity ->  userOrderMenuEnity.status == UserOrderMenuStatus.REGISTERED }
                    ?.map { userOrderMenuEnity -> userOrderMenuEnity.storeMenu }
                    ?.toList()
    ...
    fun read(
        ...
        return userOrderService.getUserOrderWithOutStatusWithInThrow(orderId, user?.id)
            .let { userOrderEntity ->
                val storeMenuEntityList = userOrderEntity.userOrderMenuList?.stream()
